...
@section('dashboard-content')
    <div class="wsus__dashboard_contant">
        <div class="wsus__dashboard_contant_top d-flex flex-wrap justify-content-between">
            ...
            <div>
                <a href="{{ route('instructor.profile.index') }}" class="common_btn">Back</a>
            </div>
        </div>
    ...
    <div class="wsus__dashboard_contant">
        <div class="wsus__dashboard_contant_top d-flex flex-wrap justify-content-between">
            <div class="wsus__dashboard_heading">
                ...
                <p>Manage payout methods and configure how instructors receive their earnings.</p>
            </div>
        </div>

        <form action="{{ route('instructor.profile.gateway-info.update') }}" method="post"
            class="wsus__dashboard_profile_update">
            @csrf
            @method('put')
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="wsus__dashboard_profile_update_info">
                        ...
                        <label>Gateway</label>
                        <select name="gateway" class="gateway form-control">
                            <option value="">Select</option>

                            @foreach ($gateways as $gateway)
                                <option value="{{ $gateway->name }}" data-id="{{ $gateway->id }}"
                                    {{ old('gateway', $gatewayInfo->gateway ?? '') == $gateway->name ? 'selected' : '' }}>
                                    {{ $gateway->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-xl-12">
                    <div class="wsus__dashboard_profile_update_info">
                        ...
                        <x-input-error :messages="$errors->get('gateway_information')" />
                        <textarea name="gateway_information" rows="7" class="gateway_description" placeholder="Your text here">{{ old('gateway_information', $gatewayInfo->gateway_information ?? '') }}</textarea>
                    </div>
                </div>
    ...
    <script>
        $(function() {
            ...
            function updatePlaceholder() {
                let id = $('.gateway').find(':selected').data('id');

                if (id) {
                    $('.gateway_description').attr(
                        'placeholder',
                        $('.gateway-' + id).text()
                    );
                }
